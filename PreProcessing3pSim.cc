...
//the string is: 1111 0000 0100 0011 0101 0100= "1pCT" in ASCII std= 15745876
    ...
    // cout<<(BegOfEvent_bits & mask)<<" ////////"<<endl;
	if((BegOfEvent_bits & mask) == 15745876){
		event_counter++;
 		Found=1;
	}else{
		bit_container= temp_cont ;
		num_bits=temp_queu-1;//I can do 4-bit steps, because the BoE usually starts at the byte or half byte beginning
	}
	return Found;
}
...
// cout<<" bits : "<<FPGAsHeader_bits<<endl;
...
unsigned int extracted_bits;      // bit containing information needed
int required_bits=0;
...
//////////////////////////////////
	...
	int FPGA_numb;
	int ASIC_address=0;
    // cout<<"MACCHECAZZO!!!! "<<__LINE__<<" strem pos "<<stream_position<<endl;
	required_bits=12;
...
//2 energy FPGAs reading
	...
	int channels=read_EnFPGAs(extracted_bits, pedestal_flag);
		...
		extracted_bits =just_read(current_bits, queued_bits,required_bits, stream_position);
        float dumb ;
        memcpy(&dumb,&extracted_bits,sizeof(extracted_bits));
        energy[GoodEventCounter][count_channel]=dumb;
		count_channel++;
