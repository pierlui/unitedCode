...
// Jul 24, 2014. Tuned for stage interfaces handling  for Jul 20 beamtest data
 class Wepl {  
   float p0[5],p0c[5],p1[5],p1c[5],p2[5],p2c[5],p3[5],p3c[5],p4[5];
	float thr0,thr1,thr2,thr3,thr4;	  

  public: 
	Wepl(float * );		//constructor
	float EtoWEPL(float *);  // returns WEPL in mm
	float myP4(float *, float);
	void SetEthresholds(float,float,float,float,float);
	
};
Wepl::Wepl(float * Fpar ) {
	for (int i=0; i<5; ++i)  {
	p0[i]=Fpar[i]; p0c[i]=Fpar[i+5];p1[i]=Fpar[i+10];p1c[i]=Fpar[i+15];
	p2[i]=Fpar[i+20]; p2c[i]=Fpar[i+25];p3[i]=Fpar[i+30];p3c[i]=Fpar[i+35];
	p4[i]=Fpar[i+40];        }
}
float Wepl::EtoWEPL(float* Estage) {
	float wet_wm,wet_pr;	
...
float Wepl::myP4(float * p,float x) { 
	float x2=x*x;
return p[0]+p[1]*x+p[2]*x2+p[3]*x2*x+p[4]*x2*x2;
}
void Wepl::SetEthresholds(float t0, float t1, float t2, float t3, float t4) {
